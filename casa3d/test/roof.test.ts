...
import { calculateRoof, roofGeometry } from '@shared/calc/roof';
...
test('isotelha em platibanda: chapas, calha, rufo e altura da platibanda', () => {
  const r = calculateRoof({ width: 10, depth: 6, type: 'platibanda', slope: 0.10, tile: 'isotelha-eps30', structure: 'aco' });
  const g = r.geometry;
  close(g.rise, 0.6);
